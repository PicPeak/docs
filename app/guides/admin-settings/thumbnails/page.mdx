...
Changing these settings and regenerating the whole library need the `settings.edit` permission; regenerating a single event needs `photos.edit` and access to that event. Without `settings.edit` the tab is read-only. A second regeneration of the same kind is refused while one is still running. (Builds containing [PicPeak PR #1483](https://github.com/PicPeak/picpeak/pull/1483).)
