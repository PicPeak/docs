...
The cost-side counterpart settings (chart of accounts, VAT codes, VAT registration, mileage / per-diem rates, the `accounting` / `incomingInvoices` / `expenses` / `taxReport` / `incomingMail` flags) live under **Settings → Accounting** — see [Admin Settings → Accounting](/guides/admin-settings/accounting).
