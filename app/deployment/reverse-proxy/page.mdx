...
<Callout type="info">
The routing below applies to the **multi-container Compose stack**, where the frontend and backend are separate services. The [all-in-one image](/deployment/single-container) serves everything from one port, so a proxy in front of it needs a single rule pointing at port `3000` — see [Example: NAS at Home](/deployment/nas-example) for a worked setup with Nginx Proxy Manager.
</Callout>
...
PicPeak keys its [API rate limiter](/guides/admin-settings/security#api-rate-limiting) and its login lockout on the client IP. Behind a proxy the backend only sees the real client address when two things hold: the proxy forwards `X-Forwarded-For` (all configurations above do), and PicPeak trusts that proxy.
...
```bash
# .env — a CIDR, a comma list, a hop count, or `true` behind a fully controlled chain
TRUST_PROXY=loopback, linklocal, uniquelocal, 203.0.113.0/24
```
...
See [Environment Variables → Core](/deployment/environment-variables#core).

Running behind Cloudflare? [Bots and Scanners](/deployment/bot-protection) has the ready-made `TRUST_PROXY` line.
