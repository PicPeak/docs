...
- **`aio:main`** is the all-in-one image on the beta channel. From v3.132.0 the all-in-one image also publishes `:stable` and `:latest` alongside `:main` / `:beta`, so on a stable install use `aio:stable`. See [Release Channels](/deployment/release-channels).
