...
The password is not written to the container log, and the credentials file is readable only by its owner (mode `0600`), so read it with `docker exec` as shown (builds containing [PicPeak PR #1491](https://github.com/PicPeak/picpeak/pull/1491)).
