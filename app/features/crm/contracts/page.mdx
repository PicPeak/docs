...
**A contract still in `draft` or `sent` that nobody has signed is cancelled and redacted.** Nothing was agreed, so there is nothing to keep.

| Data | Contract with a signature | `draft` / `sent` contract, no signature |
|---|---|---|
| Contract row and status | Kept, status unchanged | Set to `cancelled` |
...
- **The signing event log cannot be edited.** Each event's hash covers the event before it, so changing or removing a label would break the chain for every later event. Names already recorded there are retained, including on a cancelled contract.
- **Contracts in any other state are kept as they are.** A contract that was already `cancelled` before the erasure, or that the customer `declined`, is not redacted: its signer records, including a decline reason, and the customer's name and address in its text stay.
...
- The customer's activity log entry for the erasure lists which contracts were cancelled and which were kept.
