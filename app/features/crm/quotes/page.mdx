...
In builds containing [PicPeak PR #1465](https://github.com/PicPeak/picpeak/pull/1465), the customer-facing page at `/quotes/respond/<token>` first asks for a 6-digit code emailed to the customer's address on file, the same way as the [contract signing page](/features/crm/contracts#customer-signing-flow). Customers with a portal account can answer from the portal instead. After verification the page shows:
...
- A `valid until {{at}}` or `you have {{minutes}} minutes left` countdown (DE / EN respectively — see [#550](https://github.com/PicPeak/picpeak/issues/550) for the variable threading detail)
