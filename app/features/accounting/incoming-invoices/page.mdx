...
| `imap_pass` | Mailbox password. Stored masked (`********`) on GET; the real value is preserved when masked is sent back, as long as host, port, user and TLS stay the same. Changing any of them requires entering the password again — see [Mail server passwords](/features/email-notifications#mail-server-passwords) |
| `imap_folder` | Folder to poll — defaults to `INBOX`. The Settings UI offers a folder-picker that lists the available paths |

Three test actions on the page (**Test connection** and **List folders** need the `email.edit` permission):
...
### Attaching the supplier's proof

The client's invoice email can carry the stored supplier document as a **separate** attachment, so the client sees the bill behind the recharge without it being merged into your invoice PDF — invoice immutability holds either way.

It is off by default globally, can be overridden per customer, and the individual files are selectable in the send dialog. The attached filename follows a configurable template with `{INVOICE}`, `{SUPPLIER}`, `{YEAR}`, `{MONTH}` and `{SEQ}` tokens.

If a proof is missing at the moment you issue, the send is **not** blocked: the invoice goes out and the failure is recorded against the document so you can see what happened rather than discovering a silent omission later.

### Re-bills & passthrough panel

**CRM → Customer** carries a panel grouping that customer's re-bills into **Open**, **Sent** and **Paid**. The status is derived from the linked invoice's own lifecycle rather than stored twice, so it cannot drift out of sync with the invoice.

Cancelling an invoice releases the supplier costs it covered back to **Open**, so a Storno never strands a cost as permanently billed.

### Combining hours and re-bills

A cross-add dialog rolls open hours and open re-bills into a single invoice, reachable from either side. The two stay as distinct, contiguous groups of line items and are never merged into shared lines, so the client can still see what was time and what was a passed-through cost.
