...
<Callout type="info">
The editable roles, the permission matrix and the two preset roles described below are on **stable from v3.132.0**, and on beta before that. On older stable builds (v3.46.x and earlier) roles are the fixed set `super_admin`, `admin`, `editor` and `viewer` and there is no role editor. See [Release Channels](/deployment/release-channels).
</Callout>
