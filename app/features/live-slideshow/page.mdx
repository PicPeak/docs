...
- Set the **Play order**: **Chronological** (upload order, the default) or **Random (shuffle)**.
- Restrict it with **Show only category**, which defaults to **All photos**. The picker is hidden for events with no categories, and the filter is enforced on the server — including the photo count on screen.
- Override the **Gallery QR code**: **Use global default**, **On**, or **Off**.

## Gallery QR code on screen

The running slideshow can carry a QR code so guests scan the gallery straight off the projector. Turn it on under **Settings → Slideshow → Gallery QR code**, where you also set its **Position**, **Opacity (%)** and **Size (% of screen)** — bottom-left at 90% opacity and 14% size by default.

It sits in a white padded box so it stays scannable over any photo, and it shows even while the slideshow is still waiting for its first photos. It encodes the **gallery share link**, not the slideshow link, so guests reach the gallery rather than the kiosk view.
