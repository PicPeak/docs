...
- Click a row → slide-over with the exact payload sent, signature, response status, and (truncated to 1KB) response body. For roles without `settings.integrations`, the gallery `share_token` and `share_url` in that payload are masked (builds containing [PicPeak PR #1485](https://github.com/PicPeak/picpeak/pull/1485))
