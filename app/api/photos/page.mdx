...
- Entries are the stored originals, uncompressed (photos and videos are
  already compressed), named by their original upload filename. Duplicate names
  get `_1`, `_2` … suffixes.
...
- A photo whose file is missing from storage is skipped, and its id is listed
  in a `MISSING_FILES.txt` entry inside the archive.
- One request is capped at **5000 photos and 20 GiB** (by the recorded file
  sizes). Beyond that you get `400 ZIP_TOO_LARGE`; split the request with `ids`.
...
- Gallery settings that shape guest downloads — downloads disabled, the
  ...
  the originals.
- Each download leaves one entry in the admin activity log (and the
  notification bell) with the token id, event id and photo id — one entry per
  ZIP, with the photo count, rather than one per photo.
...
For a single archive instead, point one HTTP Request node at
