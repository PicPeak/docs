...
- One request is capped at **5000 photos and 20 GiB**, checked before anything
  is streamed. Sizes come from the recorded file sizes; for older photos
  without one, PicPeak asks the storage for the size. Beyond the cap you get
  `400 ZIP_TOO_LARGE`; split the request with `ids`.
- `HEAD` answers the status and headers a `GET` would, without building the
  archive.
...
**If a file fails to read while the archive is streaming**, PicPeak aborts the
connection rather than ending the response, so a truncated archive never
